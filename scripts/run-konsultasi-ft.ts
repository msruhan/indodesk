...
async function main() {
  ...
  const cases = parseAllDocTestCases().filter((c) => c.id.startsWith('FT-KON-'))
  ...
  const path = writeReport(
    generateReport({ mode: 'batch5-konsultasi-api', totalFromDocs: cases.length }),
    'batch5-konsultasi-api',
  )
