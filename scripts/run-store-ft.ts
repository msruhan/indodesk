...
async function main() {
  ...
  const cases = parseAllDocTestCases().filter((c) => c.id.startsWith('FT-STR-'))
  ...
  const path = writeReport(
    generateReport({ mode: 'batch11-teknisi-store-api', totalFromDocs: cases.length }),
    'batch11-teknisi-store-api',
  )
