...
import { resolveRekberComplaint } from '@/lib/rekber-complaint-resolve'
...
export async function POST(
  ...
  try {
    const rekber = await resolveRekberComplaint(id, session.user.id, body, {
      id: session.user.id,
    ...
    console.error('[ADMIN_REKBER_COMPLAINT_RESOLVE]', e)
