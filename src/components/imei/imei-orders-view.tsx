...
import { DashboardPageHeader, MetricCard, TabPills } from '@/components/dashboard'
...
export function ImeiOrdersView({ ordersBasePath = '/user/orders' }: ImeiOrdersViewProps) {
  ...
  const allOrders = useMemo(
    () => [
      ...
      ...serverOrders.map((o) => ({ status: o.status })),
    ],
    [orders, serverOrders],
  )
  const stats = useMemo(
    () => ({
      ...
      pending: allOrders.filter((o) => o.status === 'PENDING' || o.status === 'IN_PROCESS').length,
  ...
  const statusFilterOptions = useMemo((): FilterDropdownOption<StatusFilter>[] => {
    const source = orderTab === 'server' ? serverOrders : orders
    const countFor = (status?: ImeiOrderStatusUi) =>
      status ? source.filter((o) => o.status === status).length : source.length

    return [
      { id: 'all', label: 'Semua status', tone: 'neutral' },
      {
        id: 'PENDING',
      ...
      {
        id: 'IN_PROCESS',
        label: statusConfig.IN_PROCESS.label,
        icon: statusConfig.IN_PROCESS.icon,
        tone: 'info',
        count: countFor('IN_PROCESS'),
      },
      {
        id: 'SUCCESS',
        label: statusConfig.SUCCESS.label,
        icon: statusConfig.SUCCESS.icon,
        tone: 'success',
        count: countFor('SUCCESS'),
      },
      {
        id: 'REJECTED',
  ...
  }, [orderTab, orders, serverOrders])

  return (
    <div className="space-y-6">
      <DashboardPageHeader
        title="Riwayat Order"
        description="Lacak status order layanan perangkat dan server."
        actions={
