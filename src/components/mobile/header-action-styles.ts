/** Shared sizing for chat / bell / profile icon cluster (marketplace & dashboard). */
export const headerIconButtonClass =
  'relative inline-flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-full border border-surface-200/80 bg-white/80 text-surface-600 backdrop-blur-md transition-colors hover:border-surface-300 hover:text-ink'

export const headerIconClass = 'h-[17px] w-[17px]'

export const headerActionsGroupClass =
  'relative flex flex-shrink-0 items-center gap-1.5'

/** Mobile top bar row — same vertical rhythm as SectionTabs (marketplace). */
export const mobileHeaderBarRowClass =
  'flex items-center justify-between gap-2 px-4 py-2 sm:px-6'

/** Reserve space below fixed mobile section tabs (Top Up / actions bar). */
export const mobileSectionTabsSpacerClass =
  'h-[calc(52px+env(safe-area-inset-top,0px))] shrink-0 lg:hidden'

export const mobileHeaderBarRowDesktopClass = 'lg:h-16 lg:gap-4 lg:py-0'

export const mobileHeaderSearchInputClass =
  'h-9 rounded-full bg-white/60 pl-9 pr-3 lg:h-10 lg:pl-10 lg:pr-16'
