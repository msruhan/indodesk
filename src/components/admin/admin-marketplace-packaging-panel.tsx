...
export function AdminMarketplacePackagingPanel() {
  ...
  const load = useCallback(async () => {
    ...
    setError(null)
    try {
      const res = await fetch('/api/admin/marketplace/packaging-proofs?status=PENDING')
  ...
  const review = async (id: string, action: 'approve' | 'reject', note?: string) => {
    ...
    setError(null)
    try {
      const res = await fetch(`/api/admin/marketplace/packaging-proofs/${id}`, {
        method: 'POST',
            ...
            <CardContent className="flex flex-wrap items-center justify-between gap-3 p-4">
              <div>
                ...
                <p className="text-[11px] text-surface-500">
                  {item.media.length} file · {item.submittedAt ? new Date(item.submittedAt).toLocaleString('id-ID') : '—'}
                </p>
              </div>
