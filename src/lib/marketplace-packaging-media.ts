...
async function uploadPackagingVideo(
  orderId: string,
  ownerId: string,
  file: File,
  ...
  const folder = `packaging/${orderId}`
...
export async function uploadPackagingMediaFiles(
  orderId: string,
  ownerId: string,
  photos: File[],
  videos: File[],
  ...
  for (const file of photos) {
    const uploaded = await processUpload({
      folder: `packaging/${orderId}`,
      ownerId,
      file,
      maxBytes: MARKETPLACE_PACKAGING_PHOTO_MAX,
      visibility: 'private',
      localUrlPrefix: `/api/media/private/local/packaging/${orderId}`,
    })
    results.push({
      type: 'PHOTO',
      url: uploaded.url,
  ...
  for (const file of videos) {
    const uploaded = await uploadPackagingVideo(orderId, ownerId, file)
